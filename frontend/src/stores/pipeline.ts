...
export const usePipelineStore = defineStore('pipeline', () => {
  ...
  // Step 2
  const copyResult = ref<{ title: string; keywords: string; body: string; category?: string } | null>(null)
  const copyResults = ref<Record<number, { title: string; keywords: string; body: string; author: string; categories: string[] }>>({})
  const isGenerating = ref(false)
  const dynamicCategories = ref<string[]>([])
  const perPlatformCategories = ref<Record<number, { name: string; categories: string[] }>>({})
  ...
  const platformCovers = ref<Record<number, string>>({})  // {platform_id: cover_path}
  ...
  // Step 6
  const publishStatus = ref<Record<number, { status: string; progress: number; phase?: string; error?: string }>>({})
  ...
  async function loadTask(id: number, syncStep = true) {
    ...
    if (data.cover_candidates) {
      ...
      if (step3?.data) {
        ...
        if (stepData?.platform_covers) platformCovers.value = stepData.platform_covers
      }
    }

    // 加载 per-platform 封面路径
    for (const pt of (data.platform_tasks || [])) {
      if (pt.cover_path) platformCovers.value[pt.platform_id] = pt.cover_path
    }
    ...
    for (const pt of (data.platform_tasks || [])) {
      ...
      // 加载 per-platform 文案
      if (pt.confirmed_title) {
        let ptCats: string[] = []
        try { ptCats = JSON.parse(pt.categories || '[]') } catch {}
        copyResults.value[pt.platform_id] = {
          title: pt.confirmed_title || '', keywords: pt.confirmed_keywords || '',
          body: pt.confirmed_body || '', author: pt.author || '', categories: ptCats,
        }
      }
    }
    connectWs()
  ...
  async function loadCategories() {
    ...
    perPlatformCategories.value = data.per_platform || {}
  }
  ...
  async function generateCopy(params: Record<string, any>) {
    ...
    try {
      const result = await api('POST', `/pipeline/${taskId.value}/step/2/generate`, params)
      if (result && result.per_platform) {
        // 多平台结果：按 platform_id 映射
        const perPlatform = result.per_platform
        copyResults.value = {}
        let first: any = null
        for (const [pid, r] of Object.entries(perPlatform) as any) {
          copyResults.value[Number(pid)] = {
            title: r.title || '', keywords: r.keywords || '', body: r.body || '',
            author: r.author || '', categories: r.category ? r.category.split(',').map((c: string) => c.trim()).filter(Boolean) : [],
          }
          if (!first) first = r
        }
        // 也设置 copyResult 为第一个平台的结果（向后兼容）
        if (first) {
          copyResult.value = { title: first.title || '', keywords: first.keywords || '', body: first.body || '' }
        }
      } else if (result && result.title) {
        // 单平台结果
        copyResult.value = { title: result.title, keywords: result.keywords || '', body: result.body || '', category: result.category || '' }
        copyResults.value = {}
      }
      // 刷新完整任务数据
      try { await loadTask(taskId.value!, false) } catch {}
    } finally {
  ...
  async function confirmCopy(
    title: string, keywords: string, body: string, author: string, categories: string[],
    layoutTemplate: string = '',
    platformCopies: Array<{ platform_id: number; title: string; keywords: string; body: string; author: string; categories: string[] }> = [],
  ) {
    if (!taskId.value) return
    await api('PUT', `/pipeline/${taskId.value}/step/2/confirm`, {
      title, keywords, body, author, categories, layout_template: layoutTemplate,
      platform_copies: platformCopies,
    })
  ...
  async function generateCover(
    layout: string, candidates: number, headMargin: number = 15, size: string = '1300x640',
    platformConfigs?: Array<{ platform_id: number; platform_name: string; layout: string; head_margin: number; size: string }>,
  ) {
    if (!taskId.value) return
    const body: any = { layout, candidates, head_margin: headMargin, size }
    if (platformConfigs && platformConfigs.length > 0) {
      body.platform_configs = platformConfigs
    }
    await api('POST', `/pipeline/${taskId.value}/step/4/generate`, body)
  }

  /** 确认封面（支持多平台） */
  async function confirmCover(index: number, perPlatformCovers?: Record<number, string>) {
    if (!taskId.value) return
    const body: any = { cover_index: index }
    if (perPlatformCovers && Object.keys(perPlatformCovers).length > 0) {
      body.platform_covers = perPlatformCovers
    }
    await api('PUT', `/pipeline/${taskId.value}/step/4/confirm`, body)
  ...
  function connectWs() {
    ...
    ws.on('step_changed', (data) => {
      // 后端 advance_step 推送 to_step → 自动切换前端视图
      if (data.to_step !== undefined) {
        currentStep.value = data.to_step
      }
      // AI 生成完成/失败时清除加载态（running 状态不清除，否则动画瞬间消失）
      if (data.step === 1 && data.status !== 'running') isGenerating.value = false
    ...
    ws.on('platform_update', (data) => {
      ...
      if (data.publish_status || data.upload_progress !== undefined || data.publish_phase) {
        publishStatus.value[data.platform_id] = {
          ...publishStatus.value[data.platform_id],
          status: data.publish_status ?? publishStatus.value[data.platform_id]?.status ?? 'pending',
          progress: data.upload_progress ?? publishStatus.value[data.platform_id]?.progress ?? 0,
          phase: data.publish_phase ?? publishStatus.value[data.platform_id]?.phase,
          error: data.publish_error ?? publishStatus.value[data.platform_id]?.error,
        }
      }
    })
  ...
  function reset() {
    ...
    copyResults.value = {}
    perPlatformCategories.value = {}
    isGenerating.value = false
    renamePreview.value = []
    coverCandidates.value = []
    platformCovers.value = {}
    wmProgress.value = {}
    publishStatus.value = {}
    clearDraft()
  }
  ...
  return {
    ...
    copyResult, copyResults, isGenerating, dynamicCategories, perPlatformCategories,
    renamePrefix, renamePreview,
    coverCandidates, selectedCover, platformCovers,
