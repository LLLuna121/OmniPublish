...
const filters = ['全部','进行中','待确认','切片中','待确认切片','已完成','失败']
...
const statusMap: Record<string, { label: string; badge: string }> = {
  ...
  slice_pending:     { label: '待确认切片', badge: 'badge-orange' },
...
const miniStats = computed(() => [
  ...
  { label: '待确认切片', val: tasks.value.filter(t => t.status === 'slice_pending').length, cls: 'c-orange' },
...
function getAction(status: string) {
  ...
  if (status === 'slice_pending') return { text: '确认发布', cls: 'action-orange' }
...
function subStatusBadge(sub: any) {
  if (sub.publish_status === 'published') return { cls: 'badge-green', text: '已发布' }
  if (sub.publish_status === 'slice_pending') return { cls: 'badge-orange', text: '切片待确认' }
  if (sub.publish_status === 'failed') return { cls: 'badge-red', text: '上传失败' }
  if (sub.publish_status === 'publishing') return { cls: 'badge-primary', text: '发布中' }
...
async function handleBatchPublish() {
  ...
  try {
    const slicePendingIds = tasks.value
      .filter(t => t.status === 'slice_pending')
      .map(t => t.id)
    const data = await api('POST', '/pipeline/batch-publish', { task_ids: slicePendingIds })
    alert(`已触发 ${data.triggered || 0} 个平台的发布`)
    await loadTasks()
  } catch (e: any) {
    alert(e.response?.data?.detail || e.message || '批量发布失败')
  } finally {
...
// 重试单个平台
async function retryPlatform(taskId: number, platformId: number, skipVideo: boolean = false) {
  try {
    await api('POST', `/pipeline/${taskId}/step/6/retry`, { platform_id: platformId, skip_video: skipVideo })
    alert(skipVideo ? '已触发发布（跳过视频上传）' : '已触发重新上传')
    await loadTasks()
  } catch (e: any) {
...
<div>
  ...
  <!-- 批量操作栏 -->
  <div v-if="activeFilter === '待确认切片' && filteredTasks.length > 0"
       style="margin-bottom:16px;padding:14px 18px;background:rgba(255,183,77,.06);border:1px solid rgba(255,183,77,.2);border-radius:8px;display:flex;align-items:center;justify-content:space-between">
    <span style="font-size:13px;color:var(--orange)">
      {{ filteredTasks.length }} 个任务等待确认切片状态
    </span>
    <button class="btn btn-primary" :disabled="batchPublishing" @click="handleBatchPublish">
      {{ batchPublishing ? '发布中...' : '一键发布所有待确认任务' }}
    </button>
  </div>
      ...
      <tbody>
        <template v-for="t in filteredTasks" :key="t.id">
          <tr style="cursor:pointer" @click="toggleExpand(t.id)">
            ...
            <td class="t1 title-cell">{{ t.title || '(未设标题)' }}</td>
            <td>
            ...
            <td colspan="10" style="padding:0;background:var(--bg1)">
              ...
              <div v-for="(sub, i) in t.platform_tasks" :key="sub.platform_id" class="sub-task-row">
                ...
                <div style="display:flex;gap:8px;align-items:center;font-size:11px">
                  ...
                  <span class="step-pill" :class="sub.publish_status === 'published' ? 'done' : sub.publish_status === 'failed' || sub.publish_status === 'slice_pending' ? 'fail' : sub.publish_status === 'publishing' ? 'run' : 'wait'">发布</span>
                </div>
                <span class="badge" :class="subStatusBadge(sub).cls" style="font-size:10px">{{ subStatusBadge(sub).text }}</span>
                <div style="display:flex;gap:6px;margin-left:auto;align-items:center">
                  <button v-if="sub.publish_status === 'slice_pending'" class="btn btn-sm" style="color:var(--primary);border:1px solid var(--primary);font-size:10px;padding:2px 8px"
                          @click.stop="retryPlatform(t.id, sub.platform_id, true)">一键发布</button>
                  <button v-if="sub.publish_status === 'failed'" class="btn btn-sm" style="color:var(--orange);border:1px solid var(--orange);font-size:10px;padding:2px 8px"
                          @click.stop="retryPlatform(t.id, sub.platform_id, false)">重新上传</button>
                  <span v-if="sub.publish_error" style="font-size:10px;color:var(--red);max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" :title="sub.publish_error">{{ sub.publish_error }}</span>
                </div>
              </div>
            </td>
          </tr>
