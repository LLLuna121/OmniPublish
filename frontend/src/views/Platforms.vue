...
const form = ref({
  ...
  default_author: '', keyword_separator: ',', keyword_suffix: '',
})
...
function openEdit(p: any) {
  ...
  form.value = {
    ...
    default_author: p.default_author || '', keyword_separator: p.keyword_separator || ',',
    keyword_suffix: p.keyword_suffix || '',
  }
  initWmPreviews()
  showModal.value = true
}
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:14px">
      <div v-for="p in paged" :key="p.id"
        ...
        <div style="font-size:12px;color:var(--t2);display:flex;flex-direction:column;gap:3px">
          ...
          <div style="display:flex;justify-content:space-between"><span style="color:var(--t3)">默认作者</span><span>{{ p.default_author || '未配置' }}</span></div>
          <div style="display:flex;justify-content:space-between"><span style="color:var(--t3)">关键词分隔</span><span>{{ p.keyword_separator === '#' ? '#号' : '逗号' }}</span></div>
          <div v-if="p.keyword_suffix" style="display:flex;justify-content:space-between"><span style="color:var(--t3)">关键词后缀</span><span style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ p.keyword_suffix }}</span></div>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="showModal=false">
      <div class="modal" style="max-width:860px">
        ...
        <div class="modal-body" style="max-height:70vh;overflow-y:auto">
          ...
          <div style="display:flex;gap:14px;margin-bottom:14px">
            <div class="form-group" style="flex:1"><label>默认作者</label><input v-model="form.default_author" class="form-input" placeholder="发布时使用的作者名" /></div>
            <div class="form-group" style="flex:0.7"><label>关键词分隔符</label>
              <select v-model="form.keyword_separator" class="form-select">
                <option value=",">英文逗号 (,)</option>
                <option value="#">井号 (#)</option>
              </select>
            </div>
            <div class="form-group" style="flex:1.3"><label>关键词后缀</label><input v-model="form.keyword_suffix" class="form-input" placeholder="固定关键词1,固定关键词2（每篇自动追加）" /></div>
          </div>
          <div style="display:flex;gap:14px;margin-bottom:14px">
