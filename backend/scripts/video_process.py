...
DEFAULT_CODEC = detect_default_codec()
...
def cmd_watermark(args):
    ...
    if args.subfolders:
        ...
        for sub in sorted(input_dir.iterdir()):
            ...
            # Build a new args namespace for the subfolder (direct call, no subprocess)
            sub_args = argparse.Namespace(
                ...
                wmfile2=args.wmfile2,
                scale_landscape=args.scale_landscape,
                scale_portrait=args.scale_portrait,
                ...
                subfolders=False,  # prevent infinite recursion
            )
            cmd_watermark(sub_args)
            sub_count += 1
    ...
    for f in find_videos(args.input):
        ...
        if orient == "landscape":
            scale_ratio = args.scale_landscape
        else:
            scale_ratio = args.scale_portrait

        info(f"Processing: {name} (mode={args.wm_mode}, orient={orient}, scale={scale_ratio}%)")

        # Scale filter: shorter side to resolution
        resolution = args.resolution
...
def cmd_delogo(args):
    ...
    for f in find_videos(args.input):
        ...
        info(f"Delogo ({o}): {name}")

        vw, vh = get_dimensions(f)

        if o == "portrait":
            delogo_str = (
                "delogo=x=1:y=1:w=304:h=160,"
                "delogo=x=415:y=1:w=304:h=160,"
                "delogo=x=1:y=1119:w=304:h=160,"
                "delogo=x=415:y=1119:w=304:h=160"
            )
            if vw == 720 and vh == 1280:
                scale_filter = ""
            else:
                scale_filter = "scale=720:1280,"
        else:
            delogo_str = (
                "delogo=x=1:y=1:w=305:h=160,"
                "delogo=x=974:y=1:w=305:h=160,"
                "delogo=x=1:y=559:w=305:h=160,"
                "delogo=x=974:y=559:w=305:h=160"
            )
...
def cmd_horizontal_flip(args):
    ...
    ok(f"Horizontal flip complete: {count} videos")
...
def build_parser():
    ...
    # horizontal-flip
    sp = subparsers.add_parser("horizontal-flip", help="\u6c34\u5e73\u7ffb\u8f6c")
    add_common(sp)
...
def main():
    ...
    dispatch = {
        ...
        "delogo": cmd_delogo,
        "crop": cmd_crop,
        "horizontal-flip": cmd_horizontal_flip,
