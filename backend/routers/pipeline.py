...
async def generate_copy(task_id: int, req: GenerateCopyRequest,
    ...
    # 加载每个平台的 categories + default_author
    platform_ids = task.get("target_platforms", [])
    platforms = []
    if platform_ids:
        async with pool.acquire() as conn:
            placeholders = ",".join(f"${i+1}" for i in range(len(platform_ids)))
            rows = await conn.fetch(
                f"SELECT id, name, categories, default_author FROM platforms WHERE id IN ({placeholders})",
                *platform_ids,
            )
            for row in rows:
                platforms.append({
    ...
    if len(platforms) > 1:
        # 多平台：为每个平台独立生成差异化文案
        result = await copywrite_service.generate_per_platform(task_id, req.model_dump(), platforms)
        return ApiResponse.success(data={"per_platform": result})
    else:
        # 单平台或无平台：沿用原逻辑
        ...
        if platforms:
            ...
            if p["default_author"]:
                params["author"] = p["default_author"]
        result = await copywrite_service.generate(task_id, params)
...
async def confirm_copy(task_id: int, req: ConfirmCopyRequest, user: UserInfo = Depends(get_current_user)):
    """确认文案，推进到 Step 3。支持 per-platform 文案。"""
    task = await pipeline_service.get_task(task_id)
    if not task:
        raise HTTPException(status_code=404, detail="任务不存在")
    _check_task_access(task, user)
    ...
    if req.platform_copies:
        # 多平台独立文案：写入 platform_tasks 表，同时用第一个平台的文案写入 tasks 表作为默认值
        first = req.platform_copies[0]
        first_title = first.get("title", req.title or "")
        async with pool.acquire() as conn:
            await conn.execute(
                """UPDATE tasks SET confirmed_title = $1, confirmed_keywords = $2, confirmed_body = $3,
                   author = $4, categories = $5, rename_prefix = $6, layout_template = $7,
                   updated_at = CURRENT_TIMESTAMP WHERE id = $8""",
                first_title,
                first.get("keywords", req.keywords or ""),
                first.get("body", req.body or ""),
                first.get("author", req.author or ""),
                json.dumps(first.get("categories", req.categories or [])),
                first_title[:20],
                req.layout_template,
                task_id,
            )
            # 写入每个平台的独立文案到 platform_tasks
            for pc in req.platform_copies:
                pid = pc.get("platform_id")
                if not pid:
                    continue
                await conn.execute(
                    """UPDATE platform_tasks SET confirmed_title = $1, confirmed_keywords = $2,
                       confirmed_body = $3, author = $4, categories = $5,
                       updated_at = CURRENT_TIMESTAMP
                       WHERE task_id = $6 AND platform_id = $7""",
                    pc.get("title", ""),
                    pc.get("keywords", ""),
                    pc.get("body", ""),
                    pc.get("author", ""),
                    json.dumps(pc.get("categories", [])),
                    task_id, pid,
                )
    else:
        # 单平台模式（向后兼容）
        async with pool.acquire() as conn:
            await conn.execute(
                ...
                json.dumps(req.categories), req.title[:20], req.layout_template, task_id,
            )

    await pipeline_service.advance_step(task_id, from_step=1, to_step=2)
    await pipeline_service.add_log(task_id, f"文案已确认: {(req.platform_copies[0].get('title', '') if req.platform_copies else req.title)[:30]}", step=1)
...
class PlatformCoverConfig(BaseModel):
    platform_id: int
    platform_name: str
    layout: str = "triple"
    head_margin: float = 15
    size: str = "1300x640"

class GenerateCoverRequest(BaseModel):
    layout: str = "triple"
    head_margin: float = 15  # 百分比，前端传 15 表示 15%
    platform_configs: Optional[List[PlatformCoverConfig]] = None  # 多平台独立配置
...
async def generate_cover(task_id: int, req: GenerateCoverRequest = None,
    ...
    if req and req.platform_configs and len(req.platform_configs) > 0:
        # 多平台独立生成
        configs = [
            {"platform_id": c.platform_id, "platform_name": c.platform_name,
             "layout": c.layout, "head_margin": c.head_margin / 100, "size": c.size}
            for c in req.platform_configs
        ]
        bg.add_task(cover_service.generate_per_platform, task_id, task["folder_path"], configs)
    else:
        # 单一封面生成（兼容旧逻辑）
        layout = req.layout if req else "triple"
        head_margin = (req.head_margin if req else 15) / 100
        bg.add_task(cover_service.generate_candidates, task_id, task["folder_path"], layout, head_margin)
...
async def confirm_cover(task_id: int, req: ConfirmCoverRequest, user: UserInfo = Depends(get_current_user)):
    ...
    try:
        if req.platform_covers:
            cover_path = await cover_service.confirm_cover_per_platform(task_id, req.platform_covers)
        else:
...
async def retry_publish(task_id: int, req: RetryPublishRequest, bg: BackgroundTasks,
    ...
    bg.add_task(publish_service.retry_platform, task_id, req.platform_id, req.skip_video)
    return ApiResponse.success(message=f"平台 {req.platform_id} 重试已启动")


class BatchPublishRequest(BaseModel):
    task_ids: List[int] = []  # 空=当前用户所有 slice_pending 任务


@router.post("/batch-publish")
async def batch_publish(req: BatchPublishRequest, bg: BackgroundTasks,
                        user: UserInfo = Depends(get_current_user)):
    """批量发布所有 slice_pending 的任务（跳过视频上传，直接检查切片并发帖）。"""
    pool = await get_pool()
    async with pool.acquire() as conn:
        if req.task_ids:
            # 指定任务
            placeholders = ",".join(f"${i+1}" for i in range(len(req.task_ids)))
            rows = await conn.fetch(
                f"""SELECT pt.task_id, pt.platform_id FROM platform_tasks pt
                    JOIN tasks t ON pt.task_id = t.id
                    WHERE pt.publish_status = 'slice_pending' AND pt.task_id IN ({placeholders})
                    AND t.created_by = ${len(req.task_ids) + 1}""",
                *req.task_ids, user.id,
            )
        else:
            # 当前用户所有 slice_pending 任务
            rows = await conn.fetch(
                """SELECT pt.task_id, pt.platform_id FROM platform_tasks pt
                   JOIN tasks t ON pt.task_id = t.id
                   WHERE pt.publish_status = 'slice_pending' AND t.created_by = $1""",
                user.id,
            )

    triggered = 0
    for row in rows:
        bg.add_task(publish_service.retry_platform, row["task_id"], row["platform_id"], True)
        triggered += 1

    return ApiResponse.success(data={"triggered": triggered}, message=f"已触发 {triggered} 个平台的发布")
...
async def preview_file(task_id: int, path: str):
    """预览任务相关文件（封面、水印后图片等），仅允许访问任务素材目录内的文件。"""
    pool = await get_pool()
    async with pool.acquire() as conn:
    ...
    if not real_path.startswith(os.path.realpath(folder)):
