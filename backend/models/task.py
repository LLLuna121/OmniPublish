...
class ConfirmCopyRequest(BaseModel):
    ...
    categories: List[str] = []
    layout_template: str = ""
    platform_copies: List[dict] = []  # [{platform_id, title, keywords, body, author, categories}]
...
class ConfirmCoverRequest(BaseModel):
    """Step 4 确认：封面选择。"""
    cover_index: int = 0  # 候选封面索引（单封面模式）
    platform_covers: Optional[dict] = None  # 多平台模式: {platform_id: cover_path}
