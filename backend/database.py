...
SCHEMA_SQL = [
    ...
    CREATE TABLE IF NOT EXISTS platforms (
        ...
        keyword_separator TEXT DEFAULT ',',
        keyword_suffix    TEXT DEFAULT '',
...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        # 自动迁移：确保新增列存在（CREATE TABLE IF NOT EXISTS 不会添加新列）
        cols = await conn.fetch("PRAGMA table_info(tasks)")
        col_names = {r["name"] for r in cols}
        if "layout_template" not in col_names:
            await conn.execute("ALTER TABLE tasks ADD COLUMN layout_template TEXT DEFAULT ''")
            print("[DB] Migrated: added tasks.layout_template")

        # 迁移 platforms 表新列
        p_cols = await conn.fetch("PRAGMA table_info(platforms)")
        p_col_names = {r["name"] for r in p_cols}
        for col, default in [("default_author", "''"), ("keyword_separator", "','"), ("keyword_suffix", "''")]:
            if col not in p_col_names:
                await conn.execute(f"ALTER TABLE platforms ADD COLUMN {col} TEXT DEFAULT {default}")
                print(f"[DB] Migrated: added platforms.{col}")

        # 迁移 platform_tasks 表新列
        pt_cols = await conn.fetch("PRAGMA table_info(platform_tasks)")
        pt_col_names = {r["name"] for r in pt_cols}
        for col, default in [("confirmed_title", "''"), ("confirmed_keywords", "''"), ("confirmed_body", "''"), ("author", "''"), ("categories", "'[]'"), ("cover_path", "''"), ("cover_layout", "''")]:
            if col not in pt_col_names:
                await conn.execute(f"ALTER TABLE platform_tasks ADD COLUMN {col} TEXT DEFAULT {default}")
                print(f"[DB] Migrated: added platform_tasks.{col}")
