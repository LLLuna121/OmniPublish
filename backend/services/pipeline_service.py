...
class PipelineService:
    ...
    async def update_platform_task(self, task_id: int, platform_id: int, **kwargs):
        ...
        ALLOWED_COLUMNS = {
            ...
            "wm_video_path", "wm_cover_path",
        }
        # 仅通过 WebSocket 推送、不写入数据库的字段
        WS_ONLY_FIELDS = {"publish_phase"}
        ws_extra = {k: v for k, v in kwargs.items() if k in WS_ONLY_FIELDS}
        db_kwargs = {k: v for k, v in kwargs.items() if k not in WS_ONLY_FIELDS}
        for k in db_kwargs:
    ...
    async def compute_task_status(self, task_id: int) -> str:
        ...
        async with pool.acquire() as conn:
            ...
            failed = sum(1 for r in rows if r["publish_status"] in ("failed",))
            slice_pending = sum(1 for r in rows if r["publish_status"] == "slice_pending")
            transcoding = sum(1 for r in rows if r["transcode_status"] == "transcoding")

            if published == len(rows):
                return "done"
            if published > 0 and (failed > 0 or slice_pending > 0):
                return "partial"
            if failed == len(rows):
                return "failed"
            if slice_pending > 0:
                return "slice_pending"
            if transcoding > 0:
                return "slicing"
            return "running"
