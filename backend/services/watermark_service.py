...
from video_process import cmd_watermark as video_watermark_cmd, find_videos, detect_default_codec
...
class WatermarkService:
    ...
    async def process_all_platforms(self, task_id: int, overrides: list = None):
        ...
        if fail_count == 0:
            # 水印全部成功 → 自动推进到发布步骤
            await pipeline_service.update_step_status(
                task_id, step=4, status="done",
                data={"success_count": success_count},
            )
            await pipeline_service.add_log(
                task_id, f"水印处理完成: {success_count} 个平台全部成功，自动进入发布", step=4
            )
            await self._auto_publish(task_id)
        elif success_count > 0:
            # 部分成功 → 仍自动推进（已成功的平台可发布，失败的可稍后重试）
            await pipeline_service.update_step_status(
                task_id, step=4, status="done",
                data={"success_count": success_count, "fail_count": fail_count},
            )
            await pipeline_service.add_log(
                task_id, f"水印处理部分完成: {success_count} 成功, {fail_count} 失败，自动进入发布", step=4, level="warn"
            )
            await self._auto_publish(task_id)
        else:
            await pipeline_service.update_step_status(
    ...
    async def _process_single_platform(self, task_id: int, folder_path: str, platform: dict):
        ...
        try:
            # 输出目录：{素材文件夹}/readytopublish/{平台名}/
            safe_name = platform_name.replace(" ", "_").replace("/", "_")
            output_dir = os.path.join(folder_path, "readytopublish", safe_name)

            has_img_wm = bool(platform.get("img_wm_file"))
            has_vid_wm = bool(platform.get("vid_wm_file"))
            img_wm_file = platform.get("img_wm_file", "")
            vid_wm_file = platform.get("vid_wm_file", "")

            if not has_img_wm and not has_vid_wm:
                # 无任何水印文件：跳过，直接标记完成
                await pipeline_service.update_platform_task(
                    task_id, platform_id,
                    wm_status="done", wm_progress=100,
                    wm_images_dir=folder_path,
                )
                await ws_manager.send_to_task(task_id, {
                    "type": "platform_update",
            ...
            # ── 图片水印 ──
            if has_img_wm and os.path.exists(img_wm_file):
                await asyncio.to_thread(
                    watermark_images,
                    folder=folder_path,
                    output=output_dir,
                    watermark_path=img_wm_file,
                    img_width=800,
                ...
                await ws_manager.send_to_task(task_id, {
                    ...
                    "platform_id": platform_id,
                    "platform_name": platform_name,
                    "wm_status": "running", "wm_progress": 50,
                    "wm_images_count": img_count,
                })
                await pipeline_service.add_log(
                    task_id, f"{platform_name}: 图片水印完成 ({img_count} 张)", step=4, platform_id=platform_id
                )

            # ── 视频水印 ──
            if has_vid_wm and os.path.exists(vid_wm_file):
                # 检查素材文件夹是否有视频
                videos = find_videos(folder_path)
                if videos:
                    import argparse
                    vid_output_dir = output_dir  # 视频输出到同一目录
                    os.makedirs(vid_output_dir, exist_ok=True)

                    codec = detect_default_codec()
                    vid_args = argparse.Namespace(
                        input=folder_path,
                        output=vid_output_dir,
                        watermark=vid_wm_file,
                        wm_mode=platform.get("vid_wm_mode", "corner-cycle"),
                        fixed_pos="bottom-right",
                        wmfile2=platform.get("vid_wm_file2", ""),
                        scale_landscape=platform.get("vid_wm_scale", 35),
                        scale_portrait=platform.get("vid_wm_scale", 35),
                        resolution="720",
                        codec=codec,
                        bitrate="2M",
                        fps=30,
                    ...
                    await pipeline_service.add_log(
                        task_id, f"{platform_name}: 开始视频水印 ({len(videos)} 个视频, mode={vid_args.wm_mode})",
                    ...
                    vid_files = [f for f in os.listdir(vid_output_dir)
                                 if os.path.splitext(f)[1].lower() in vid_exts] if os.path.isdir(vid_output_dir) else []
                    vid_count = len(vid_files)
                    if vid_files:
                        vid_out_path = os.path.join(vid_output_dir, vid_files[0])

                    await pipeline_service.add_log(
                        task_id, f"{platform_name}: 视频水印完成 ({vid_count} 个视频)", step=4, platform_id=platform_id
                    )
            ...
            if vid_out_path:
                update_fields["wm_video_path"] = vid_out_path

            await pipeline_service.update_platform_task(
                task_id, platform_id, **update_fields
            )
            await ws_manager.send_to_task(task_id, {
                "type": "platform_update",
                "platform_id": platform_id,
                "platform_name": platform_name,
                "wm_status": "done", "wm_progress": 100,
                "wm_images_count": img_count, "wm_videos_count": vid_count,
            })
            await pipeline_service.add_log(
                task_id, f"{platform_name}: 水印处理完成 ({img_count} 张图片, {vid_count} 个视频)",
                step=4, platform_id=platform_id
            )
    ...
    async def _auto_publish(self, task_id: int):
        """水印完成后自动推进步骤并触发发布。"""
        try:
            # 推进: step 4 (水印) → step 5 (上传&发布)
            await pipeline_service.advance_step(task_id, from_step=4, to_step=5)
            await pipeline_service.add_log(task_id, "自动推进到上传&发布步骤", step=5)
