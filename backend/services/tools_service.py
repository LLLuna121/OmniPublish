...
class ToolsService:
    ...
    async def horizontal_flip(self, input_dir: str, output_dir: str = "",
                              codec: str = "libx264", bitrate: str = "2M") -> str:
        """水平翻转视频。"""
        job = self._register("horizontal_flip", {k: v for k, v in locals().items() if k != 'self'})
        asyncio.create_task(self._run_video_cmd(job, "horizontal-flip", input_dir, output_dir, codec=codec, bitrate=bitrate))
        return job.id
